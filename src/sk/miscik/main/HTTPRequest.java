 ...
 */
// 194.160.229.181:8081
public class HTTPRequest {

    public static final String IP_ADDRESS = "192.168.0.110:8081";
    ...
    public Applicant[] getInterviews(User user, int start, int limit) throws Exception {
        ...
        for (Object obj1 : list) {
            ...
            String status = (String) interview.get("status");

            applicants[i] = new Applicant(id, firstName,lastName,email,phone,city,room,position,date, status);
    ...
    public void closeEntry(User user, Applicant applicant) throws Exception {
        String url = "http://"+HTTPRequest.IP_ADDRESS+"/api/interviews/"+applicant.getID()+"/closed";
        if (user == null) throw new NullPointerException("user=null");
        URL obj = new URL(url);
        HttpURLConnection con = (HttpURLConnection) obj.openConnection();
        con.addRequestProperty("Authorization","Bearer "+user.getToken());
    ...
    public void deleteCandidate(User user, Applicant applicant) throws Exception {
        String url = "http://"+HTTPRequest.IP_ADDRESS+"/api/interviews/"+applicant.getID();
        if (user == null) throw new NullPointerException("user=null");
        URL obj = new URL(url);
        HttpURLConnection con = (HttpURLConnection) obj.openConnection();
        con.addRequestProperty("Authorization","Bearer "+user.getToken());
