package sk.miscik.gui;

import com.github.lgooddatepicker.components.DateTimePicker;
import com.github.lgooddatepicker.optionalusertools.DateTimeChangeListener;
import com.github.lgooddatepicker.zinternaltools.DateTimeChangeEvent;

import javax.swing.*;
import java.awt.*;
import java.time.*;
import java.util.Calendar;
import java.util.Date;

public class DateTimeDialog extends JDialog {

    private Date date;

    public DateTimeDialog(Frame frame) {
        super(frame,"Choose date" ,true);
        this.date = new Date();

        this.setResizable(false);
        this.getContentPane().setMinimumSize(new Dimension(400,70));
        this.getContentPane().setPreferredSize(new Dimension(400,70));
        this.getContentPane().setMaximumSize(new Dimension(400,70));
        this.getContentPane().setBackground(new Color(57,57,71));
        this.getContentPane().setLayout(null);

        DateTimePicker picker = new DateTimePicker();
        picker.setSize(380, 50);
        picker.setLocation(10,10);
        picker.addDateTimeChangeListener(dateTimeChangeEvent -> {
            ZonedDateTime zdt = dateTimeChangeEvent.getNewDateTimeStrict().atZone(ZoneId.of("Europe/Bratislava"));
            this.date = new Date(zdt.toInstant().toEpochMilli());
        });
        this.getContentPane().add(picker);

        this.pack();
        this.setVisible(true);
    }

    public Date getSelectedDate() {
        return date;
    }

    public void setDate(Date date) {
        this.date = date;
    }
}
